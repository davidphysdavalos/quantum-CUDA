...
__global__ void Ui_kernel(int n,int m,double R[],double I[],double mcos,double msin,int l) {
  long long int index=threadIdx.x + blockIdx.x * blockDim.x;
  long long int in=ipow(2,n);
  long long int im=ipow(2,m);
  while (index<l) {
    Complex i(0,1);
...
//   if (cacheindex==0) {
//     matomicAdd(&densR[(tarjet1*ndim)+tarjet2],cache.real);
//     matomicAdd(&densI[(tarjet1*ndim)+tarjet2],cache.imag);
//   }
}
